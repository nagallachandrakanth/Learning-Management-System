...
public class AdminController {
	...
	public ApiResponse<String> rejectRequest(@PathVariable("employeeId") Integer employeeId,
		...
		if (rejectRequst.isPresent()) {
			rejectEmail.sendingEmail("chandramailsender@gmail.com", rejectRequst.get().getEmailId(),
					rejectRequst.get().getMessage());
			return new ApiResponse<String>("employee request rejected", null, null);
		}
		throw new RuntimeException("employee request has been hold");
	}

	@GetMapping(path = "/batchlist")
	public ApiResponse<Object> batchList() {
		Optional<List<BatchDto>> batchlist = adminService.batchlist();
		if (batchlist.isPresent()) {
			return new ApiResponse<Object>("batch list data", null, batchlist.get());
		}
		throw new RuntimeException("no data found");
	}

	@GetMapping(path = "/mentorlist")
	public ApiResponse<Object> mentorList() {
		Optional<List<MentorDto>> mentorList = adminService.mentorList();
		if (mentorList.isPresent()) {
			return new ApiResponse<Object>("Mentor list", null, mentorList.get());
		}
		throw new RuntimeException("mentor list not available");
	}

	@GetMapping(path = "/requestlist")
	public ApiResponse<Object> requestList() {
		Optional<List<RequestDto>> requestList = adminService.requestList();
		if (requestList.isPresent()) {
			return new ApiResponse<Object>("Request Data list", null, requestList.get());
		}
		throw new RuntimeException(" request list is not available");
	}
