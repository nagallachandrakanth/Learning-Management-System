...
public class AdminServiceImpl implements AdminService {
	...
	public Optional<List<MentorDto>> mentorList() {
		List<Mentor> mentorFromDb = mentorRepository.findAll();
		if (mentorFromDb != null) {
			List<MentorDto> mentorDto = new ArrayList<MentorDto>();
			for (Mentor mentorEntity : mentorFromDb) {
				MentorDto mentorDtoCopy = new MentorDto();
				BeanUtils.copyProperties(mentorEntity, mentorDtoCopy);
				List<MentorTechnologiesDto> mentorTechnologiesDtos = new ArrayList<MentorTechnologiesDto>();

				for (MentorTechnologies mentorTechnologies : mentorEntity.getSkills()) {
					MentorTechnologiesDto mentorTechnologiesDto = new MentorTechnologiesDto();
					BeanUtils.copyProperties(mentorTechnologies, mentorTechnologiesDto);
					mentorTechnologiesDtos.add(mentorTechnologiesDto);
				}
				mentorDtoCopy.setSkillsDto(mentorTechnologiesDtos);
				mentorDto.add(mentorDtoCopy);
			}
			return Optional.ofNullable(mentorDto);
		}
		return Optional.ofNullable(null);
	...
	@Override
	public Optional<List<RequestDto>> requestList() {
		List<Request> requestList = requestRepository.findAll();
		if (requestList != null) {
			List<RequestDto> requestDto = new ArrayList<RequestDto>();
			for (Request requestEntity : requestList) {
				RequestDto requestDtoCopy = new RequestDto();
				BeanUtils.copyProperties(requestEntity, requestDtoCopy);
				requestDto.add(requestDtoCopy);
